...
verdict 之后按 `omo-plan-structure` 的判定附件格式，对每个实施 task 输出一行三类判定；判定是**执行期输入而非阻断项**，不改变官方 verdict 格式与阻断语义，**随 verdict 一并输出、不因输出精简纪律省略**：

- **tdd（测试时序）**：判据——测试是「定义行为」还是「确认行为」。四问：①不读实现能否写测试 ②失败是否静默 ③是否语义变更/修复 ④是否仅模式复制。命中前三任一 → `first`（Atlas 派发时注入先红后绿指令）；仅模式复制或其余 → `after`。
- **split（拆解与并发重组）**：逐 impl task 按原子性契约做反事实拆分——能拆出两个以上可独立发布、独立验收、独立回退的结果、且各中间状态都能独立通过时判 `yes` 并给出拆分边界与各自验收；被无证据串行化的 task（对照并行准入六条件全部满足）判 `yes` 并给出 cohort 归属或 wave 重组；捆绑不同产品 owner 或 failure family 且无有效「非原子」举证的 task 默认判 `yes`。「非原子」举证成立的合并不因「还能拆」被推翻，须指出举证三要素中具体失效一项才判 `yes`；超出 lane 上限或启动税大于收益的维持串行。
- **route（路由合适性）**：对照 `omo-plan-structure`「路由档位判据」核对——标注明显失当（档位过高浪费或过低不可执行）时写判定值；一致时照抄计划标注。判定覆盖计划标注，Atlas preflight 采纳。
- 判定随首次审查输出；复审委托载明增量范围时，仅对新增 task 补判定，不重做全量。
