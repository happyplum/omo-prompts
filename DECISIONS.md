...
- 修订（2026-09-05，repair-plan 对齐重写）：`commands/repair-plan.md` 按 D-027/D-030/D-032/D-039/D-040 对齐现行体系（计划分级与判级行、Task 5 字段、checkbox 即进度、上游六段、`/ulw-execute`、计划/账本分离）；上游契约自检清单收敛——增 `omo-plan-structure`（结构单一标准，D-022），删 momus 派发行（测试时序已由审查判定附件承载，D-030，派子代理不属于「加载」）；历次「本仓 / 两仓无残留」类验收的作用域自此包含 commands 仓。同日二次修订：自检清单再收敛为 **4 个 skill**，删除 `prompts/atlas.md`、`prompts/prometheus.md` 两项「直接读源」——角色 prompt 是角色行为文件，command 读取会把他人角色条款注入执行上下文（与 D-029 防劫持同类污染）；计划对齐契约以 `omo-plan-structure` 为准，workspaces 身份与 handoff 要素由关卡条目自足承载。
...
### D-046 Atlas 派发前并行盘点（就绪集成组派发）

- 状态：`active`
- 决策（2026-08-31 用户指令「Atlas 派发任务前需要关注哪些任务可以并行一起派发」；系 D-005/D-008 预算 fan-out 语义在 D-039 回归上游逐 task 验收节奏后的动作化补强）：`atlas.md`「原子化与并发」的 cohort 派发条款升级为**并行盘点**——每波启动与每次补位派发前，先从并发矩阵识别当前就绪集（硬前驱满足、写域互斥、接口决策明确），把其中互不依赖的 task 作为一组在同一回合并行发出（各派独立子代理、按并发预算分批、默认 `run_in_background=true`）；执行期 REMAP 或再拆分后重算就绪集再派发；就绪集多成员时不得无证据退化为逐个串行派发，串行仅限真实依赖（后继消费前驱产物、共享写域或未冻结接口）。WAVE-READY 与 `WHY_NOT_PARALLEL` 判据以 `omo-adaptive-execution` 为单一来源，`atlas.md` 只承载执行纪律。
- 验收：`atlas.md` 含并行盘点条款与串行退化反向约束；与 D-039 逐 task 验证节奏无冲突（验收逐个做、派发并行发）；canary 按 D-011 待首个多就绪 task 计划执行时补（度量：单回合并行派发数、就绪集并行率）。

### D-047 全局规范新增「心智负担与单源原则」

- 状态：`active`
- 决策（2026-09-03 用户指令完善 AGENTS.md、减少开发后期心智负担；流程：Metis 风险审查 R1-R10 → Oracle 交叉裁决 11 处 → Metis 终审 GO → 用户追加三轮裁决）：`runtime/AGENTS.md` 两处——
  1. **优先级链插入第 4 层「单一事实源与心智负担」**（用户真实意图与任务边界之下、长期载体一致性之上）：单源收敛压过最小改动与效率，不压过正确性、安全与用户意图。
  2. **新增「心智负担与单源原则」章节 8 条**：心智负担优先／单一事实源（副本须可由权威源重建，多可写源须写明同步与冲突裁决）／共享实现优先（同一业务语义+行为/错误处理/边界条件一致才共享，共享后验证各调用方行为不变）／边界一次归一化（按进入内部模型的边界各归一化一次，存量回退到当前改动授权范围最外层数据进入点）／中间层成立条件（能消除重复或隔离外部格式差异才新增）／柯里化按场景使用（API 要求、框架库惯例、多调用方复用预绑定均合法，禁多层闭包隐藏依赖/状态/执行时机）／过渡层必须可删除（跨任务须登记删除对象与验收点到产品规格或长期计划）／删除与新增同等重要（存在被替代对象而无旧源退出视为未收敛）。另于「原子派发」补并行所有权：涉及同一共享实现、可写权威源或迁移收敛的改动不得拆给并行子代理，由一个 task 统一负责。
  3. **用户追加裁决**：手段类禁令改边界式（禁同一数据流内重复归一化、中间层按成立条件判断、柯里化三合法场景+唯一禁令，判断线显式）；越界收敛走确认机制——及时询问+附收益评估（重复规模、影响面），收益明显时经确认后扩大范围（L26 总则，L32 迁移场景引用）。
- 验收：`runtime/AGENTS.md` 优先级链含新层级、新章节 8 条齐备、「原子派发」含并行所有权句；`sync-agents.ps1 -Check` 两份一致。

### D-048 Metis 目的差异分析与输入契约

- 状态：`active`
- 决策（2026-09-04 用户裁决：Metis 除意图分析外须稳定提供目的分析——目的着重点/描述清晰度/符合度；分析所需的对话、内容与证据全部由调用方提供，「哪里调用就在哪里写明附带哪些证据」，拥有证据才能分析；触发默认仅用户显式要求、用户明确拒绝即跳过）：`metis.md` 重构为四节——
  1. **目的差异分析三判据**（单一来源在 `metis.md`，调用方不复制判据）：目的着重点＝从用户多轮陈述原话提取真实优先级，显式声明与隐含排序分列、陈述冲突点名交调用方裁决；描述清晰度＝原话歧义逐条（原话摘录＋至少两种合理解读＋被分析方实际采用解读＋偏移判定）；符合度＝用户陈述逐条对照被分析方文本/产物（符合/偏离/未覆盖/不可判，附两端证据锚）。
  2. **输入契约 fail-closed**：用户原话按轮次引号摘录（转述须标注、不与原话混排）；被分析方文本为原文摘录，不凭标题或复述替代；最低输入——着重点≥两轮原话、清晰度＝原话＋解读文本、符合度＝陈述条目＋被分析方文本；不足或与声称对象不符即返回所需证据清单停止，不做推测性分析。
  3. **报告不提问**：差异分析输出报告、不发起消解提问（提问纪律仅约束意图分析）；输出按分析类型最小结构承载（排序清单/歧义逐条/映射表），每条判定附原话锚。
  4. **调用点写明证据**（`metis.md` 不经 skill 加载进入调用方上下文，证据清单必须落在调用方文件；调用点「输入要求见 `metis.md` 输入契约」为**有效指引**：输入契约约束的是派发方组装 metis 委托证据的行为，调用方以**读源**方式对照契约——读源与 skill 加载是两个机制，不得以「调用方不加载 metis.md」为由判其为死引用，2026-09-05 评审曾误判后经用户澄清恢复）：`sisyphus.md` L0 表新增目的核对的显式触发入口行；`prometheus.md`「需求与目标」节新增目的核对入口（未成稿附当前方案要点、成稿附计划对应节，被采纳结论落入「需求与目标」或计划对应节）；`omo-atlas-execution-constraints` 完成审查补证据基准（用户需求原话或计划「需求与目标」节摘录＋产物摘要），范围偏差核对自此有对照基准，同节补显式触发入口（skills 仓 SK-020 同裁决）。
- 验收：`metis.md` 含三判据与「输入契约」节（fail-closed）；三个调用点含证据清单与触发权条款；判据与契约在 `metis.md` 之外无重复维护；canary 按 D-011 待首次真实差异分析委托补（度量：输入一次通过率、fail-closed 拒绝次数、判定证据锚覆盖率）。
- 修订（2026-09-05，用户裁决，经两轮修正定稿）：第 2 点「输入契约」保留于 `metis.md`（响应方验收标准：合格输入判定与不足退回）；skills 仓 `omo-adaptive-execution`「委托契约」节另设「Metis 目的差异分析传参契约」（SK-021，派发方组装规范：如何为 metis 提供合格资料；Sisyphus/Atlas 加载天然可见、Prometheus 读源）——**传参契约与输入契约是同一委托接口的两端，各自自包含、内容对应，不视为重复维护**；三调用点引用改指传参契约；fail-closed 语义与最低输入标准不变。
