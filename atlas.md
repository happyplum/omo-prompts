...
- **起点门禁**：在 `subagent-driven-development` 与 `atlas-execution-constraints` 这条预加载链闭合前，Atlas 不得开始任何 `task()` 委托、路由判断，或执行 TODO surface 展开；若发现自己已经在无链状态下进入执行，必须立即停止并回退到补链/修复路径。
