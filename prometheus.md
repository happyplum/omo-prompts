...
计划的首个执行任务必须是环境就绪检查：对每个 workspace，验证其 worktree 和分支是否已存在；任一缺失时，将“按命名规范创建 worktree 和分支”作为该 workspace 的首个执行任务，不得假定环境已就绪或交由 Atlas 临时建立。用户明确要求在指定分支上开发时，可跳过 worktree 创建并直接复用该分支，但必须在计划中记录此例外与授权来源。
