...
计划开篇必须有「需求与目标」节，记录与用户讨论确认的本计划目标、硬约束、非目标与验收标准来源。溯源规则：
...
- 共享同一推理、核心不变量、未冻结接口、循环依赖或只能整体验收的工作 → 单一强 owner。
- 有向依赖 → pipeline 排列，只并行同一反链。
- parallel wave → 仅当至少两个 task 均可独立产出、独立失败和独立验收时才考虑。
- 缺少会实质改变目标、范围、实现或验收的决策时返回 `BLOCKED_NEEDS_DECISION`，不得自行补齐。

### 原子单位（全局约定）

- 一个可独立发布、回退与验收的行为意图 + 一组 owned 产物；实现、直接测试与使该行为成立所必需的调用方必须同属一个 task/todo；不按操作步骤拆成微任务。
...
任一条件不成立即改为 single-owner 或 pipeline，不按文件数量机械拆分。
...
- 每个 Task 契约中的 task 必须在矩阵中恰好出现一次；硬前驱必须可解析到已知 task；缺失、漏项或不可解析即结构无效（矩阵含依赖必须无环），审查按官方 Executability 类别阻断。
- 拓扑豁免：单 writer 单 lane（允许串行多个写入 task）可写 `cohorts: none` 代替表格。
- cohort 是并行归属而非物理派发批次：实际分批由 Atlas 按并发预算执行，分批不改变归属。
- 预算口径：运行中写入 worker 与未验收积压之和默认 ≤ 3，隔离充分至 4；矩阵可声明 `concurrency_budget: N`，声明时以计划值为准（预算体制的唯一覆盖入口，与 shared skills、atlas 三方一致）。
...
- 硬前驱与仅集成关联
- owner
- 冻结验收契约 `acceptance_contract`：每条含稳定 ID、二元条件、证据取得方式与证据作用域文件清单；计划批准后计算 `checklist_hash`；执行期修订一律 append-only——新增条目用新 ID，既有条目 ID 与 hash 不变
- 允许输入
- 唯一可写产物
- 禁止范围
- 环境 preflight
...
- 高价路由（`unspecified-high` / `deep` / `ultrabrain` / `artistry`）必须写 `WHY_NOT_LOWER_COST`；前台执行独立 ready 写入任务必须写 `WHY_NOT_PARALLEL`。
...
计划必须显式包含检查点声明；检查点是计划唯一验收节点来源，Atlas 的验收节奏直接挂靠，不再另设重复节点：
...
- `mode: current` 必须记录 `authorization_source`，指向用户对使用当前工作区的明确授权；普通计划批准、工作区看似干净或规划者判断均不算授权，且保留现有分支。
...
- task 路由标注：每个可提前确定路由的 task 写明 `category` 或 `subagent_type` 二选一及 `load_skills`；无法确定时标注带原因的 `executor_judgment`，且不得同时指定 `category`/`subagent_type`。该标记表示 Atlas 必须在 dispatch 前按 `omo-adaptive-execution` 解析并记录唯一最终路由及理由，不是空缺占位。
...
- 高精度审查把同一已保存计划版本交给 Momus 与 Oracle，并以两者均返回 `OKAY` 为通过条件。
...
- 计划正文只含五个静态区块，以二级标题组织并保持结构可机械判定：需求与目标、Workspaces、并发矩阵、Task 契约、检查点与集成（基线预验证据以单行记入「检查点与集成」）；静态区块置顶，保持前缀稳定以命中 prompt caching。
