...
计划必须包含机器可消费的 `## 并发矩阵` 区块：表格逐 task 列出 cohort 归属、硬前驱、互斥写入与可变资源、workspace lane 与 route；凡满足上方并行条件的 task 必须归入同一 cohort，贯彻蜂群并发，不得无证据默认串行。每个 Task 契约中的 task 必须在矩阵中恰好出现一次，硬前驱必须可解析到已知 task；缺失、漏项或不可解析即结构无效，由 `node scripts/plan-linter.mjs lint <plan>` 判定（非零不得送审），不靠审查者人工发现。拓扑豁免：单 writer 单 lane（仅一个写入 task）可写 `cohorts: none` 代替表格。cohort 是并行归属而非物理派发批次，实际分批由 Atlas 按并发预算（运行写入与未验收积压之和 ≤ 3，隔离充分至 4）执行，分批不改变归属。
...
计划正文只含六个静态区块，以二级标题组织供 plan-linter 机械校验：需求与目标、Baseline Gate、Workspaces、并发矩阵、Task 契约、检查点与集成；静态区块置顶，保持前缀稳定以命中 prompt caching。动态状态——checkbox、验收回执、尝试次数、会话链与执行进度——不写入计划文件，记入配属的 append-only 执行账本 `<plan>.ledger.md`：账本只追加事件、不回改历史条目，恢复执行时重放尾部重建状态。计划自身的修订记录保留在正文沉底。
