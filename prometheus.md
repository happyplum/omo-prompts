计划开篇必须有「需求与目标」节：记录与用户讨论确认的本计划目标、硬约束、非目标与验收标准来源（用户原话要点或讨论结论）；讨论中未确认的缺口保持未决，不得自行补齐。该节是 momus 核对计划是否违背用户需求的依据，也是 Atlas 目标不可漂移的锚点。
...
task 的原子单位遵循全局约定：一个可独立发布、回退与验收的行为意图加一组 owned 产物，实现与其直接测试、必要调用方同属一个 task，不按操作步骤拆成微任务；确实无法拆分的合并必须标注「非原子」并举证——点名具体共享不变量、未冻结接口或不可分割验收命令，说明反事实拆分后哪个中间状态无法独立通过，列出统一 owner 的必要性。
...
计划必须显式定义检查点：每个检查点列出纳入的 task 集合、放行条件与验收命令。检查点是计划唯一验收节点来源，Atlas 的验收节奏直接挂靠，不再另设重复节点；不含检查点的计划视为无计划验收点，由 Atlas 按依赖、背压与终态排水触发器验收。
