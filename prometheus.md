...
- 命中独立 reviewer 条件的 task 在 Task 契约中显式标注 `reviewer 安排`：审核者路由档位与审核范围；审核输入为冻结验收契约原文注入，通过条件为逐条 `PASS`。未标注视为低风险父协调者验收。未通过的回退修复由执行方通用规则持有（原会话定向修复、修复后重验），计划不逐 task 重复。
